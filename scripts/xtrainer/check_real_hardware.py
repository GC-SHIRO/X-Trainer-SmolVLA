...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if baseline is not None and current is not None:
            _LOGGER.info("Restoring observed baseline pose")
            with contextlib.suppress(Exception):
                current = _move_gripper(environment, current, 6, float(baseline[6]), 0.0)
                current = _move_gripper(environment, current, 13, float(baseline[13]), 0.0)
            with contextlib.suppress(Exception):
