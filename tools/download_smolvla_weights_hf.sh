...
usage() {
  cat <<'USAGE'
Usage: bash tools/download_smolvla_weights_hf.sh [OPTIONS]

Download the SmolVLA policy and its VLM backbone from Hugging Face.

Options:
  --repo-id ID          Model ID (default: lerobot/smolvla_base)
  --output-dir PATH     Local model directory (default: models/smolvla_base)
  --vlm-repo-id ID      VLM model ID (default: HuggingFaceTB/SmolVLM2-500M-Video-Instruct)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --vlm-repo-id) VLM_REPO_ID="${2:?--vlm-repo-id requires a value}"; shift 2 ;;
...
printf '[xtrainer-download-hf] policy: %s\n' "${REPO_ID}"
printf '[xtrainer-download-hf] VLM: %s\n' "${VLM_REPO_ID}"
printf '[xtrainer-download-hf] endpoint: %s\n' "${ENDPOINT}"
printf '[xtrainer-download-hf] policy destination: %s\n' "${OUTPUT_DIR}"
printf '[xtrainer-download-hf] VLM destination: %s\n' "${VLM_OUTPUT_DIR}"

conda run --no-capture-output -n "${ENV_NAME}" \
  python - "${REPO_ID}" "${OUTPUT_DIR}" "${VLM_REPO_ID}" "${VLM_OUTPUT_DIR}" "${REVISION}" "${ENDPOINT}" <<'PY'
import os
import sys

from huggingface_hub import snapshot_download

policy_repo_id, policy_dir, vlm_repo_id, vlm_dir, revision, endpoint = sys.argv[1:]
for repo_id, output_dir in ((policy_repo_id, policy_dir), (vlm_repo_id, vlm_dir)):
